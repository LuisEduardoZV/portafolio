  ...
  return (
    <>
      <MainCard>
        ...
        <MainContentCard className='p-0 sm:p-5 sm:w-11/12 mb-1 sm:mb-4'>
          <section className='flex w-full h-fit sm:gap-x-5 items-center justify-between sm:justify-normal pl-5 sm:pl-0'>
            ...
            <div className='flex w-full flex-col'>
              <FormattedMessage id='menu-item-bluetooth' />
              <label className='hidden sm:block text-sm text-acc-main sm:text-xs lg:text-base'>{active ? 'Ahora visible como "MocBaack NoAir".' : 'Localización, NoAirDrop y No AirPlay usan Bluetlooth'}</label>
            </div>
        ...
        <label className='flex pl-5 sm:hidden w-full text-sm text-main-text-low sm:text-acc-main sm:text-xs lg:text-base'>{active ? 'Este NoIphone etará visible como "NoIphone de Luis" mientras la configuración de Bluetooth esté abierta.' : 'Localización, NoAirDrop y No AirPlay usan Bluetlooth'}</label>
        <div className={`flex w-full h-full mt-8 items-center gap-x-2 ${!active ? 'opacity-0' : 'opacity-100'} transition-opacity duration-0  sm:w-11/12`}>
          <label className='text-main-text pl-5 sm:pl-1 font-semibold uppercase sm:normal-case text-sm sm:text-base'>Mis dispositivos</label>
        </div>
        ...
        <div className={`flex w-full h-full mt-8 items-center gap-x-2 ${!active ? 'opacity-0' : 'opacity-100'} transition-opacity duration-0  sm:w-11/12`}>
          <label className='text-main-text pl-5 sm:pl-1 font-semibold uppercase sm:normal-case text-sm sm:text-base'>Dispositivos cercanos</label>
        ...
        <MainContentCard className={`p-5 duration-0 relative gap-y-10 ${active ? 'opacity-100' : 'opacity-0'} sm:w-11/12`}>
          <label className={`${active ? 'opacity-0 absolute invisible' : 'opacity-100 relative visible'} transition-visible duration-100 ease-linear delay-[1200ms] top-10 left-1/2 -translate-x-1/2 -translate-y-1/2 hidden sm:block`}>Buscando...</label>
